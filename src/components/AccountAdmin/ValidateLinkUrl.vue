...
export default {
    ...
    methods: {
        ...
        validateLinkGuid() {
            ...
            dataService.validateLinkUrl(this.linkType, this.linkGuid).then(response => {
                this.waiting = false;
                let responseData = response.data;
                if (responseData.statusCode == 200) {
                    let link = responseData.registrationLinkDetails[0];
                    if (link.isLinkActive.toLowerCase() == 'yes')
                    {
                        this.validationResult.isValid = true;
                        this.validationResult.linkDetails = responseData.registrationLinkDetails[0];
                    } else {
